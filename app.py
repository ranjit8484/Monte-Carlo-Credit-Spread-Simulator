...
target_profit_min = st.sidebar.number_input("Target Profit Min ($ per trade)", min_value=100, max_value=700, value=200)
target_profit_max = st.sidebar.number_input("Target Profit Max ($ per trade)", min_value=100, max_value=700, value=400)
...
max_contracts_pct = st.sidebar.slider("Max Exposure (% of Account)", min_value=10, max_value=50, value=50)
...
def simulate_trade(account, delta, qty, spread, profit_min, profit_max):
    ...
    credit_per_contract = total_collateral * delta / 100
    ...
    if win:
        realized_profit = np.random.uniform(profit_min, profit_max)
        realized_profit = min(realized_profit, total_collateral)
        loss = 0.0
        rollover_needed = 0.0
    else:
    ...
    return account + realized_profit - loss, realized_profit, loss, rollover_needed, win, total_collateral, total_credit, qty

def simulate_rollover(account, debit_to_cover, spread, delta, max_contracts_allowed, profit_min, profit_max):
    collateral_per_contract = spread * 100
    credit_per_contract = collateral_per_contract * delta / 100.0
    needed_qty = max(1, min(int(np.ceil(debit_to_cover / credit_per_contract)), max_contracts_allowed))
    total_collateral = needed_qty * collateral_per_contract
    total_credit = needed_qty * credit_per_contract
    ...
    if win:
        ...
        realized_profit = 0.0
        loss = total_collateral
    account = account + realized_profit - loss

    return account, realized_profit, loss, win, needed_qty, total_collateral, total_credit
...
for sim in range(num_simulations):
    ...
    all_sim_results.append({
        ...
        "Avg Rollovers per Trade": (total_rollovers / float(total_trades)) if total_trades > 0 else 0.0,
        "Max Contracts Used": max_contracts_used,
        "Total Trades": total_trades,
        "Wins Before Roll": wins_before,
        "Wins After Roll": wins_after,
        "Max Drawdown": max(initial_account - np.array(account_history))
    })
...
# Mean trajectory with 10-90 percentile
...
fig_traj = go.Figure()
fig_traj.add_trace(go.Scatter(x=x_axis + x_axis[::-1], y=list(p90) + list(p10[::-1]), fill='toself', fillcolor='rgba(173,216,230,0.3)',
                              line=dict(color='rgba(255,255,255,0)'), hoverinfo="skip", name='10-90 percentile'))
fig_traj.add_trace(go.Scatter(x=x_axis, y=mean_traj, mode='lines', name='Mean trajectory', line=dict(color='royalblue', width=3)))
...
# ---------------- Trade Table ----------------
st.header("Trade Details by Simulation")
sim_selected = st.selectbox("Select Simulation Number", options=range(1, num_simulations+1))
sim_trade_df = trade_df[trade_df["Simulation"]==sim_selected]
st.dataframe(sim_trade_df.reset_index(drop=True))
