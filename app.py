...
def simulate_trade(account, delta, qty, spread, target_profit):
    ...
    # Correct probability: higher delta → lower chance of winning
...
def simulate_rollover(account, debt_to_cover, spread, delta, qty_multiplier, max_contracts_allowed, target_profit):
    """Simulate a rollover trade, covering previous loss"""
    credit_per_contract = spread * (delta / 100) * 100
    needed_qty = int(np.ceil(debt_to_cover / credit_per_contract))
    ...
    if win:
        # Gain covers previous debt up to credit received
        account += min(total_credit, debt_to_cover)
        loss = 0
    else:
        # Only lose remaining uncovered amount
        loss = max(0, debt_to_cover - total_credit)
        account -= loss

    # Realized profit for dashboard metrics
    realized_profit = total_credit if win else 0
